...
hamar_kommune <- read_pq_data("gclists/hamar_kommune.gpx") %>% 
...
missing_finds_per_year <- missing_finds %>% 
  ...
  tidyr::complete(year = 2001:2020, nesting(Kommune), fill = list(antall = 0)) %>% 
...
#  dplyr::filter(type == "Unknown Cache") %>% 
  dplyr::select(gc_code) %>% 
  dplyr::mutate(solved = TRUE)

missing_finds_mysteries <- missing_finds %>% 
  dplyr::filter(type == "Unknown Cache") %>% 
  dplyr::left_join(solved_mysteries, by = "gc_code") %>% 
  dplyr::mutate(solved = dplyr::if_else(is.na(solved), FALSE, TRUE))

missing_mysteries_summarized <- missing_finds_mysteries %>% 
  dplyr::group_by(Kommune, solved) %>% 
  dplyr::summarise(antall = n()) %>% 
  tidyr::complete(solved = c(TRUE, FALSE), nesting(Kommune), fill = list(antall = 0)) %>% 
  tidyr::pivot_wider(names_from = Kommune, values_from = antall)
```
